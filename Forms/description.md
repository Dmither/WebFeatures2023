...
**Поле вводу** `input` відображається залежно від `[type=""]`  
...
**Випадаючий список** `select`, елементи задаються `option`.  
`[size=""]` - кількість відображених варіантів.
`[multiple]` - можливість вибору кількох варіантів.
`option`: обов'язковий атрибут `[value=""]` передається в форму.  
`[selected]` - варіант за умовчуванням.

**Текстова область** `textarea` - багаторядкове поле вводу.  
`[cols="30"]` та `[rows="10"]` визначають розмір в символах.  

**Кнопка** `button`, `[type=""]` визначає функціонал за умовчуванням.
